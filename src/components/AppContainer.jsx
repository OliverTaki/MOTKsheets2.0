...
const MainView = ({ sheets, fields, columnWidths, onColumnResize, activeFilters, onFilterChange, allShots, sortKey, ascending, onSort, visibleFieldIds, onVisibilityChange, onAddField, onCellSave, onUpdateFieldOptions, onUpdateNonUuidIds, idToColIndex }) => {
  return (
...
export const AppContainer = () => {
  ...
  const handleCellSave = useCallback(async (shotId, fieldId, newValue, idToColIndex) => {
    ...
    // +2 because Google Sheets are 1-indexed and there are two header rows
  ...
  const handleUpdateNonUuidIds = useCallback(async () => {
    ...
    try {
        ...
        refreshData(); // Refresh data to show new IDs
    } catch (err) {
